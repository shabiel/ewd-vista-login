...
module.exports.handlers.getUsers = function(messageObj, session, send, finished) {
  let query = messageObj.params.query;
  // Replace HTML encoding for plain characters
  // Commas
  query     = query.replace('%2C', ',');
  console.log('Query:' + query);
  
  let result = this.db.function({
    function: 'LIST^ewdVistAFileman',
    arguments: [200, '', '@;.01;1', 'PQ', '', query, query, '', '', '']
  });
  
  let usersNode = new this.documentStore.DocumentNode('TMP', ['DILIST', process.pid]);
  let usersData = this.handlers['ewd-vista'].convertDILISTToArray(usersNode);
  console.log('Search results:');
  console.log(usersData);
  usersData     = usersData['IEN^.01^1^2']
  
  // if (usersData.length == 0) { usersData = [query]}
  
  finished({users: usersData});
}