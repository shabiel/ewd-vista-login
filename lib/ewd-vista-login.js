...
module.exports.handlers.isLogonInhibited = function(messageObj, session, send, finished) {
  ...
  // IP Address and Client Name for VistA
  this.db.symbolTable.restore(session);
  this.db.symbolTable.set('XWBTIP', session.ipAddress);
  this.db.symbolTable.set('XWBCLMAN', this.db.version());
...
module.exports.handlers.cvc = function(messageObj, session, send, finished) {
  ...
  // Load configuration
  if (isConfigOptionSet('upper-case-vc')) {
...
module.exports.handlers.getAuthorizedModules = function(messageObj, session, send, finished) {
  ...
  for (var i = 0; i < modulesData.length; i++) {
    ...
    if (securityKeys && securityKeys.length) {
...
module.exports.handlers.getFixtures = function(messageObj, session, send, finished) {
  let fixtures = {};

  if (isConfigOptionSet('dev')) {
    fixtures = require('ewd-vista-login/test/fixtures.json');
  }
...
// Private methods
function isConfigOptionSet(option) {
  let config   = require('ewd-vista-login/package.json').ewdVista.config;
