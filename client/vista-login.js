...
clientMethods.createKeyboardShortcuts = function(EWD) {
  ...
  $(window).on('keydown', function(e) {
    ...
    if (e.ctrlKey && e.key == 'M') { e.preventDefault(); $('#apps-menu li.dropdown a.dropdown-toggle').click(); return; }
      ...
      modulesData.forEach(function(element) {
        ...
        $('#app-' + element.htmlName).click(function(e) {
          ...
          $.getScript('assets/javascripts/' + element.htmlName + '.js', function(){
            ...
            if (window[element.htmlName]) window[element.htmlName].prep(EWD);
            else if (window[element.clientModuleName]) window[element.clientModuleName].prep(EWD);
            else (console.error('No click handler defined for menu'));
          });
        });
      });
    });
